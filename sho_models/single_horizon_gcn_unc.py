...
# Setup Logging
# ---------------------------
LOG_DIR = os.path.join(MAIN_PATH, "output/sho/logs_gcn_unc")
OUTPUT_DIR = os.path.join(MAIN_PATH, "output/sho/study_results_gcn_unc")
...
def objective(trial: optuna.Trial):
    try:
        ...
        train_loader, val_loader, test_loader = prepare_data(use_validation=use_validation, prediction_horizon=DIRECT_HORIZONS_TO_PREDICT, batch_size=params["batch_size"])
...
def main():
    try:
        ...
        output_file = os.path.join(OUTPUT_DIR, f"sho_gcn_trials_df.parquet")
        trials_df.to_parquet(output_file)

        with open(OUTPUT_DIR + '/sho_gcn_study.pickle', 'wb') as handle:
